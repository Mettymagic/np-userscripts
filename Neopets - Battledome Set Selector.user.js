...
if(window.location.href.includes("/tvw/")) {
    ...
    function addLootDisplay() {
        let lootcont = document.createElement("bdlootcont")
        lootcont.id = "bdlootcont"
        $("#bdPopupGeneric-winnar")[0].appendChild(lootcont)

        addLootBars() //adds display for prize tally
        addRewardList() //adds the list of rewards
    }
    ...
    function addRewardList() {
        ...
        $("#bdlootcont")[0].appendChild(table)
    ...
    function addLootBars() {
        ...
        if(isTVW) var bar3 = bar1.cloneNode(true)
        //item bar
        let w1 = GM_getValue("bdloottrack", {items:0, np:0}).items
        let imax = isTVW ? PLOT_MAX_ITEMS : MAX_ITEMS
        bar1.querySelector(".lootprogress-bar").style.backgroundColor = "#1E90FF"
        bar1.querySelector(".lootprogress-bar").style.width = `${Math.min(w1/imax*100.0, 100)}%`
        bar1.querySelector(".lootprogress-text").innerHTML = `${w1} / ${imax} Items`
        //np bar
        ...
        if(isTVW) {
            let w3 = GM_getValue("plotpoints", 0)
        ...
        $("#bdlootcont")[0].appendChild(barCont)
    ...
    function countRewards() {
        ...
        if(isTVW) {
            let pp = $("#bd_rewards > div.rewardsContainer > p > span.prizname")
            if(pp.length > 0) {
                var earned = parseInt(pp[0].innerHTML.split(" ")[0])
                let prev = GM_getValue("plotpoints", 0)
                GM_setValue("plotpoints", prev + earned)
                console.log(`[BD+] ${earned} plot points earned, recorded.`)
            }
        }
